...
$Pendingtickets = mysqli_query($conn,
"SELECT COUNT(*) as all_total FROM tickets WHERE status='Pending' AND email='$email'");

$availableTicket = mysqli_fetch_assoc($Pendingtickets);




$total_tickets = mysqli_query($conn,
"SELECT COUNT(*) as all_total FROM tickets WHERE email='$email'");

$submitted_tickets = mysqli_fetch_assoc($total_tickets);

$result = mysqli_query($conn,
"SELECT * FROM tickets WHERE  email='$email' ORDER BY created_at DESC");
?>
  ...
  <div class="card" style="margin-bottom: 20px;">
    ...
    <div class="card-body">
      ...
      <?php if (mysqli_num_rows($result) > 0)  { ?>
        ...
        <?php } ?>
        <?php }else{ ?>
           <p style="color:var(--text-muted); font-size:13px; text-align:center; padding:14px 16px; background:var(--bg); border:1.5px dashed var(--border); border-radius:8px;">No tickets submitted yet.</p>
        <?php } ?>
      </div>
    </div>
          ...
          <div class="stat-row">
            <div class="stat">
              ...
              <div class="stat-value" id="stat-total">
                 <?php echo $submitted_tickets['all_total']; ?>
              </div>
            </div>
            <div class="stat">
